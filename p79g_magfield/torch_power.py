...
class crossspectrum:
    def __init__(self, arr1: torch.Tensor, arr2: torch.Tensor):
        """
        arr: Tensor of shape [B, C, N, M] (2D) or [B, C, N, N, N] (3D)
        """
        assert arr1.ndim in (4, 5), "Input must be [B,C,N,M] or [B,C,N,N,N]"
        device = arr1.device
        rank = arr1.ndim - 2  # 2 for 2D, 3 for 3D

        arr1 = arr1.to(torch.float32)
        arr2 = arr2.to(torch.float32)

        # FFT over spatial dimensions only
        Nhat1 = torch.fft.fftn(arr1, dim=tuple(range(-rank, 0)))
        Nhat1 /= arr1[ 0, 0].numel()  # normalize by number of points
        Nhat2 = torch.fft.fftn(arr2, dim=tuple(range(-rank, 0)))
        Nhat2 /= arr2[ 0, 0].numel()  # normalize by number of points
        rhohat = torch.real( Nhat1*torch.conj(Nhat2))

        nz = rhohat.shape[-1]
        kx = torch.fft.fftfreq(nz, device=device) * nz  # <<< device-aware
        self.dk = kx[1] - kx[0]
        self.dx = 1 / arr1.shape[-1]
        ...
        power_per_bin = torch.zeros(
            (arr1.shape[0], arr1.shape[1], len(bins) - 1),
            dtype=rhohat.dtype,
            device=device
        )
