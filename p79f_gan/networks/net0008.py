...
idd = 8
what = "GAN 7 with a bunch of high pass stuff"
...
def train_gan(
    ...
    D = Discriminator(cond_dim=cond_dim, ch=ch, in_ch=2, im_size=im_size).to(device)
    ...
    for step in tqdm.tqdm(steps):
        ...
        with autocast(device, enabled=use_amp):
            c = mach_embed(mach)
            x_real_D = torch.cat([x_real, laplacian(x_real)], dim=1)

            z = torch.randn(x_real.size(0), z_dim, device=device)
            x_fake = G(z, c).detach()
            #x_fake = aug_turbulence(x_fake)

            x_fake_D = torch.cat([x_fake, laplacian(x_fake)], dim=1)
            d_real, _ = D(x_real_D, c)
            d_fake, _ = D(x_fake_D, c)
...
# inside your G step, after x_fake computed
            ps,logPf, logPr  = power_spectrum_loss(x_fake.float(), x_real.float(), BIN_IDX, NBINS, remove_dc=True)
            l1 = (x_fake - x_real).abs().mean()
            gpdf, pdf_f, pdf_r, bins = grad_pdf_loss(x_fake, x_real, nbins=64)
            phase_loss=fft2_phase_loss(x_real.float(),x_fake.float(),eps=1e-4)
            bi, bf, br = bispectrum_loss(x_fake, x_real, BIN_IDX, PAIRS)

            lossG = g_hinge_loss(d_fake) + 0.1*l1 + lambda_ps*ps + 0.1*phase_loss + 0.8*bi + 0.8*gpdf
        ...
        if step % 20 == 0:
            ...
            plt.subplot(3,2,4); plt.plot(k_centers[1:].cpu(),logPr[0].cpu().detach().numpy()); plt.xscale('log'); plt.ylim(ymin,ymax)
            plt.yscale('log')
            plt.subplot(3,2,4); plt.plot(k_centers[1:].cpu(),logPf[0].cpu().detach().numpy()); plt.xscale('log'); plt.ylim(ymin,ymax)
            plt.title('Power spectra')
            plt.yscale('log')
            plt.subplot(3,2,5)
            plt.plot(bins.cpu().detach().numpy(), pdf_r[0].cpu().detach().numpy())
            plt.plot(bins.cpu().detach().numpy(), pdf_f[0].cpu().detach().numpy())
            plt.xscale('log')
            plt.tight_layout()
