...
if 'm' not in dir():
    m = hp.read_map(mapfile, field=0)
    #Q = hp.read_map(mapfile, field=1)
    #U = hp.read_map(mapfile, field=2)
    mE = hp.read_map("data/E_map.fits", field=0)
    mB = hp.read_map("data/B_map.fits", field=0)
...
subset=''
i=0
allthings = torch.cat([cutoutE,cutoutB])
mean = allthings.mean()
std = allthings.std()
#cutout = (cutout-mean)/std
cutoutE = (cutoutE-mean)/std
cutoutB = (cutoutB-mean)/std
