...
idd = 9005
what = "Hybrid CNN-ViT with Uncertainty Quantification (mean + variance prediction)"
...
def load_data():
    ...
    valid_raw = loader.loader(dirpath + fname_valid, ntrain=1, nvalid=0)
...
def thisnet():
    model = HybridCNNViT(
        ...
        in_chans=3,
        num_classes=2,  # Mean + log variance
        embed_dim=embed_dim,
        depth=depth,
        num_heads=num_heads,
        mlp_ratio=mlp_ratio,
...
class HybridCNNViT(nn.Module):
    """
    Hybrid CNN-ViT with Uncertainty Quantification.
    
    Outputs: (mean, log_variance) for each prediction
    This allows computing confidence intervals: mean ± k*sqrt(exp(logvar))
    """
    def __init__(self, img_size=128, cnn_output_size=64, patch_size=8, 
                 in_chans=3, num_classes=2, embed_dim=384, depth=6, 
        ...
        # Head outputs 2 values: mean and log variance
    ...
    def forward(self, x):
        ...
        mean = out[:, 0:1]  # [B, 1]
        logvar = out[:, 1:2].clamp(-10, 5)  # [B, 1] - clamp for stability
    ...
    def criterion1(self, pred, target):
        ...
        # We drop constants and optimize: 0.5 * (logvar + (y - μ)² * exp(-logvar))
        nll = 0.5 * (logvar + (target - mean)**2 * torch.exp(-logvar))
        
        # Also track MSE for comparison
        mse = F.mse_loss(mean, target)
        
        return {'nll': nll.mean(), 'mse': mse}

    def criterion(self, pred, target):
        """Main loss function: Gaussian NLL with mild high-Mach weighting"""
        mean, logvar = pred
        
        # Base NLL loss
        nll = 0.5 * (logvar + (target - mean)**2 * torch.exp(-logvar)).mean()
        
        # Mild weighting for high Mach (Ms > 10)
        weights = torch.where(target > 10.0, 1.5, 1.0)
        weighted_mse = (weights * (mean - target)**2).mean()
        
        # Combine: primarily NLL (learns uncertainty), small weighted MSE
        return 0.9 * nll + 0.1 * weighted_mse
...
def trainer(model, all_data, epochs=50, batch_size=64, lr=5e-4,
    ...
    print(f"Architecture: CNN + ViT with Uncertainty Estimation (mean ± σ)")
...
def plot_loss_curve(model):
    ...
    ax.set_title(f'Training Curves - net{idd} (Hybrid CNN-ViT + Uncertainty)', fontsize=14)
