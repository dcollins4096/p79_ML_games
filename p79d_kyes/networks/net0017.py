...
class main_net(nn.Module):
    def __init__(self, in_channels=1, out_channels=2, base_channels=32, hidden_dim=512, fc_spatial=8):
        ...
        self.enc1 = nn.Conv2d(in_channels, base_channels, kernel_size=3, stride=1, padding=1)   # same res
        self.enc2 = nn.Conv2d(base_channels, base_channels*2, kernel_size=3, stride=2, padding=1) # /2
        self.enc3 = nn.Conv2d(base_channels*2, base_channels*4, kernel_size=3, stride=2, padding=1) # /4
        ...
        # ----- Decoder -----
        self.up3 = nn.ConvTranspose2d(base_channels*4, base_channels*2, kernel_size=2, stride=2) # ×2
        self.dec3 = nn.Conv2d(base_channels*2, base_channels*2, kernel_size=3, padding=1)

        self.up2 = nn.ConvTranspose2d(base_channels*2, base_channels, kernel_size=2, stride=2)   # ×2
        self.dec2 = nn.Conv2d(base_channels, base_channels, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        out = self.dec1(torch.cat([d2, x], dim=1))  # match with input
        return out
