...
class main_net(nn.Module):
    def __init__(self, in_channels=1, out_channels=2, base_channels=32, hidden_dim=512, fc_spatial=8):
        super().__init__()
        self.fc_spatial = fc_spatial

        # ----- Encoder (stride convs for downsampling) -----
        self.enc1 = nn.Conv2d(in_channels, base_channels, kernel_size=3, stride=1, padding=1)   # H, W
        self.enc2 = nn.Conv2d(base_channels, base_channels*2, kernel_size=3, stride=2, padding=1) # H/2, W/2
        self.enc3 = nn.Conv2d(base_channels*2, base_channels*4, kernel_size=3, stride=2, padding=1) # H/4, W/4
        ...
        # ----- Decoder -----
        self.up3 = nn.ConvTranspose2d(base_channels*4, base_channels*2, kernel_size=2, stride=2) # → H/2
        self.dec3 = nn.Conv2d(base_channels*4, base_channels*2, kernel_size=3, padding=1)  # concat with e2

        self.up2 = nn.ConvTranspose2d(base_channels*2, base_channels, kernel_size=2, stride=2)   # → H
        self.dec2 = nn.Conv2d(base_channels*2, base_channels, kernel_size=3, padding=1)  # concat with e1

        # Final layer, concat with raw input
        self.dec1 = nn.Conv2d(base_channels + in_channels, out_channels, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        e2 = F.relu(self.enc2(e1))  # [B, 2*base, H/2, W/2]
        e3 = F.relu(self.enc3(e2))  # [B, 4*base, H/4, W/4]

        # ----- Bottleneck -----
        B, C, H, W = e3.shape
        z = F.adaptive_avg_pool2d(e3, (self.fc_spatial, self.fc_spatial))
        z = z.view(B, -1)
        z = F.relu(self.fc1(z))
        z = F.relu(self.fc2(z))
        ...
        d3 = F.relu(self.up3(z))            # [B, 2*base, H/2, W/2]
        d3 = torch.cat([d3, e2], dim=1)     # concat skip
        d3 = F.relu(self.dec3(d3))

        d2 = F.relu(self.up2(d3))           # [B, base, H, W]
        d2 = torch.cat([d2, e1], dim=1)     # concat skip
        d2 = F.relu(self.dec2(d2))

        out = self.dec1(torch.cat([d2, x], dim=1))  # final skip from raw input
        return out
