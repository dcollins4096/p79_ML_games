...
idd = 22
what = "net 4 without the interpolation step, take 2"
...
class main_net(nn.Module):
    def __init__(self, Npix=128, fc_hidden=256):
        super().__init__()
        self.Npix = Npix
        ...
        self.flattened_size = 128 * Npix * Npix
        self.fc1 = nn.Linear(self.flattened_size, fc_hidden)
        self.fc2 = nn.Linear(fc_hidden, self.flattened_size)
        
        # Decoder convolution to reduce channels to 2
        self.output_conv = nn.Conv2d(128, 2, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        x = x_flat.view(batch_size, 128, self.Npix, self.Npix)
        
        # Output conv to produce 2-channel output
