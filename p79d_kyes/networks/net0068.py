...
class main_net(nn.Module):
    ...
    def criterion(self, pred, target):
        ...
        pixel_loss = F.l1_loss(pred, target[:,0:1,:,:])
        spectral_loss = fft_loss(pred, target[:,0:1,:,:])
        a = torch.cos(self.fourier_weight)**2
        actual = a*pixel_loss+(1-a)*spectral_loss
        return actual
