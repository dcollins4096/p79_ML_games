...
idd = 108
what = "100 with more decoder capacity"
...
class DoubleResidualBlockSE(nn.Module):
    def __init__(self, in_channels, out_channels, reduction=16):
        super().__init__()
        self.block1 = ResidualBlockSE(in_channels, out_channels, reduction)
        self.block2 = ResidualBlockSE(out_channels, out_channels, reduction)
...
class main_net(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, base_channels=32,
        ...
        # Optional FC bottleneck (unchanged)
        ...
        # --- Decoder upgrades ---
        # Larger kernel size for smoother upsampling & bigger receptive field
        self.up4 = nn.ConvTranspose2d(base_channels*8, base_channels*8, kernel_size=5, stride=2, padding=2, output_padding=1)
        self.up3 = nn.ConvTranspose2d(base_channels*8, base_channels*4, kernel_size=5, stride=2, padding=2, output_padding=1)
        self.up2 = nn.ConvTranspose2d(base_channels*4, base_channels*2, kernel_size=5, stride=2, padding=2, output_padding=1)

        # Fix: explicitly define channel sizes so we don't mismatch
        up4_out = base_channels * 8   # must match self.up4 out_channels
        up3_out = base_channels * 4   # must match self.up3 out_channels
        up2_out = base_channels * 2   # must match self.up2 out_channels

        self.up4 = nn.ConvTranspose2d(base_channels*8, up4_out, kernel_size=5, stride=2, padding=2, output_padding=1)
        self.up3 = nn.ConvTranspose2d(up4_out, up3_out, kernel_size=5, stride=2, padding=2, output_padding=1)
        self.up2 = nn.ConvTranspose2d(up3_out, up2_out, kernel_size=5, stride=2, padding=2, output_padding=1)

        # Decoder blocks now use those variables
        self.dec4 = DoubleResidualBlockSE(up4_out + base_channels*4, up4_out)
        self.dec3 = DoubleResidualBlockSE(up3_out + base_channels*2, up3_out)
        self.dec2 = DoubleResidualBlockSE(up2_out + base_channels,   up2_out)
        self.dec1 = nn.Conv2d(up2_out, out_channels, 3, padding=1)

        # Multi-scale heads
        self.out_d4 = nn.Conv2d(up4_out, out_channels, 3, padding=1)
        self.out_d3 = nn.Conv2d(up3_out, out_channels, 3, padding=1)
        self.out_d2 = nn.Conv2d(up2_out, out_channels, 3, padding=1)
