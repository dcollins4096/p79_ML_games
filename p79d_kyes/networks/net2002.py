...
idd = 2002
what = "180 with the normalizing flow head.  Try spatial encoding"
    ...
    def forward(self, features, target=None, mode="train"):
        ...
        if mode == "train":
            ...
            with torch.no_grad():
                ...
                sample0, _ = self.flow._transform.inverse(z0, context=context)  # mode-ish residual
                # reshape back to [B,2,H,W]
            BHW = y.shape[0]
            B = features.shape[0]
            # you already know H,W; cache them
            sample_res = sample0.view(B, H, W, 2).permute(0,3,1,2)
            # Spectral (very light) penalty to cut high-k noise
            tv = (sample_res[:, :, :, 1:] - sample_res[:, :, :, :-1]).abs().mean() + \
                 (sample_res[:, :, 1:, :] - sample_res[:, :, :-1, :]).abs().mean()
            loss = -log_prob.mean() + 0.5*F.mse_loss(sample0, y) + 1e-4*tv
            return loss
...
class main_net(nn.Module):
    ...
    def criterion1(self, preds, target):
        ...
        res = target[:, 1:, :, :]-out_main[:,1:,:,:]  # E,B channels
        flow_loss = self.flow_head(features, target=res, mode="train")
