...
idd = 176
what = "168, planck data, try to learn, more losses"

fname_train = "planck_subsets_2000setsSmooth5_half0.h5"
fname_valid = "planck_subsets_2000setsSmooth5_half1.h5"
#ntrain = 2000
...
def load_data():
    ...
    #all_data={'train':train['train'],'valid':valid['valid'], 'test':train['train'], 'quantities':{}}
    if 'quantities' in train:
...
class SphericalDataset(Dataset):
    def __init__(self, all_data, rotation_prob = 0.0):
        ...
        if normalize:
            # Compute global mean/std over all samples and channels
            self.mean = self.all_data.mean(dim=(0, 2, 3), keepdim=True)
            self.std = self.all_data.std(dim=(0, 2, 3), keepdim=True)
            # Avoid divide-by-zero in case of uniform channels
...
class main_net(nn.Module):
    def __init__(self, in_channels=1, out_channels=3, base_channels=32,
                 ...
                 err_L1=1, err_Multi=1,err_Pear=1,err_SSIM=0,err_Grad=0,err_Power=1,err_Bisp=0,
