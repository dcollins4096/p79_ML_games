...
idd = 2011
what = "2008 with another new head"
...
class EBFlowHead(nn.Module):
    """
    Conditional flow modeling [T,E,B] directly.
    Includes SSIM + power-spectrum losses for extra structure guidance.
    """

    def __init__(self, in_channels, context_dim=64, hidden_features=128,
                 num_layers=6, edge_width=8,
                 w_ssim=0.1, w_power=0.05):
        super().__init__()
        ...
        self.context_net = nn.Sequential(
            nn.Conv2d(in_channels, 64, 3, padding=1, padding_mode="reflect"),
        ...
        def make_coupling():
            ...
            def make_net(*args, **kwargs):
                in_features, out_features = args[0], args[1]
                return IgnoreContextNet(in_features, out_features)
    ...
    def forward(self, features, target=None, mode="train"):
        ...
        if mode == "train":
            ...
            # mode (MAP) sample for perceptual terms
            ...
            # light smoothness regularizer
            tv = (
                (sample_res[:, :, :, 1:] - sample_res[:, :, :, :-1]).abs().mean()
                + (sample_res[:, :, 1:, :] - sample_res[:, :, :-1, :]).abs().mean()
            ) * 1e-5

            loss = nll + self.w_ssim * ssim_term + self.w_power * pwr_term + tv
            return loss
