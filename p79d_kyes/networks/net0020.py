...
class main_net(nn.Module):
    def __init__(self, in_channels=1, out_channels=2, base_channels=32, hidden_dim=1024, fc_spatial=4):
        super().__init__()
        ...
        self.dec3a = nn.Conv2d(base_channels*2, base_channels*2, 3, padding=1)
        self.dec2 = nn.Conv2d(base_channels*4, base_channels, 3, padding=1)
        self.dec2a = nn.Conv2d(base_channels, base_channels, 3, padding=1)
        self.dec1 = nn.Conv2d(base_channels*2 + in_channels, out_channels, 3, padding=1)
    ...
    def forward(self, x):
        ...
        # Residual bottleneck connection
        z = z + e3

        # ----- Decoder with skip connections -----
        # Stage 3: skip with e2
        d3 = F.relu(self.up(z))
        ...
        # Stage 1: final skip with raw input
        x_skip = F.interpolate(x, size=(d2.shape[2], d2.shape[3]), mode='bilinear', align_corners=False)
        out = self.dec1(torch.cat([d2, x_skip], dim=1))
