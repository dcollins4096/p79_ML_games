...
class main_net(nn.Module):
    ...
    def criterion(self, preds, target):
        ...
        if 0:
            ...
            #return total_loss + ploss
            #total_loss = loss_main + 0.5 * loss_d2 + 0.25 * loss_d3 + 0.125 * loss_d4
        ploss = pearson_2channel(out_main, target)
        l1=F.l1_loss(out_main, target)
        return l1 + ploss
def pearson_2channel(pred,target):
    p1 = pred[:,0,:,:]
    p2 = pred[:,1,:,:]
    t1 = pred[:,0,:,:]
    t2 = pred[:,1,:,:]
    out = pearson_r_loss(p1,t1)+pearson_r_loss(p2,t2)
    return out

def pearson_r_loss(pred, target, eps=1e-8):
    pred_mean = torch.mean(pred)
    target_mean = torch.mean(target)
    vx = pred - pred_mean
    vy = target - target_mean
    corr = torch.sum(vx * vy) / (torch.sqrt(torch.sum(vx ** 2) + eps) *
                                 torch.sqrt(torch.sum(vy ** 2) + eps))
    return 1 - corr  # want to maximize correlation, so minimize (1 - r)
