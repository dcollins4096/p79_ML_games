...
idd = 3104
what = "3102 with rotation and normalization"
...
fname_train = "p79d_subsets_S512_N5_xyz__down_64T_annfix_first.h5"
fname_valid = "p79d_subsets_S512_N5_xyz__down_64T_annfix_second.h5"

fname_train = "p79d_subsets_S512_N7_xyz__down_128T_annfix_first.h5"
fname_train = "p79d_subsets_S512_N7_xyz__down_128T_annfix_second.h5"
...
def thisnet():
    ...
    model = main_net(base_channels=32,fc_hidden=2048 , fc_spatial=4, use_fc_bottleneck=fc_bottleneck, out_channels=3, use_cross_attention=False, attn_heads=1, rotation_prob=1.0)
...
class SphericalDataset(Dataset):
    def __init__(self, all_data, quan, rotation_prob = 0.0, mean=None,std=None):
        ...
        self.all_data = np.log(self.all_data)
        if mean is None:
            self.mean = self.all_data.mean(dim=(0,2,3), keepdim=True)
            self.std  = self.all_data.std(dim=(0,2,3), keepdim=True) + 1e-8
        else:
            self.mean = mean
            self.std  = std
        self.all_data = (self.all_data-self.mean)/self.std
    ...
    def __getitem__(self, idx):
        ...
        x=theset[0].to(device)
        if random.random() < self.rotation_prob:
            # 0, 90, 180, 270 deg
            k = random.randint(0, 3)
            x = torch.rot90(x, k, dims=(-2, -1))

            # maybe also random flips
            if random.random() < 0.5:
                x = torch.flip(x, dims=[-1])
            if random.random() < 0.5:
                x = torch.flip(x, dims=[-2])

        return x, torch.tensor([ms], dtype=torch.float32).to(device)
...
def trainer(
    ...
    ds_val   = SphericalDataset(all_data['valid'],all_data['quantities']['valid'], rotation_prob=model.rotation_prob, mean=ds_train.mean,std=ds_train.std)
    ...
    print("Total Steps", total_steps, "ntrain", len(all_data['train']))
...
class main_net(nn.Module):
    def __init__(self, in_channels=1, out_channels=3, base_channels=32,
        ...
        if self.predict_scalars:
            ...
            self.fc_out = nn.Sequential(nn.Linear(in_dim,in_dim),nn.ReLU(),nn.Dropout(p=0.1), nn.Linear(in_dim, self.n_scalars))
