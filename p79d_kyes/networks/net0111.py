...
idd = 111
what = "net 100 with dropout and capacity"
...
# ---------------- Residual SE Block with Dropout ----------------
class ResidualBlockSE(nn.Module):
    def __init__(self, in_channels, out_channels, reduction=16, dropout_p=0.0):
        super().__init__()
...
class main_net(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, base_channels=32,
        ...
        self.enc1 = ResidualBlockSE(in_channels, base_channels, dropout_p=0.1)
        self.enc2 = ResidualBlockSE(base_channels, base_channels * 2, dropout_p=0.1)
        self.enc3 = ResidualBlockSE(base_channels * 2, base_channels * 4, dropout_p=0.2)
        self.enc4 = ResidualBlockSE(base_channels * 4, base_channels * 8, dropout_p=0.3)
        ...
        self.dec4 = ResidualBlockSE(base_channels * 8 + base_channels * 4, base_channels * 4, dropout_p=0.2)
        self.dec3 = ResidualBlockSE(base_channels * 4 + base_channels * 2, base_channels * 2, dropout_p=0.1)
        self.dec2 = ResidualBlockSE(base_channels * 2 + base_channels, base_channels, dropout_p=0.05)

        self.final_dropout_p = 0.05  # small regularization before final output
        self.dec1 = nn.Conv2d(base_channels, out_channels, 3, padding=1)

        self.register_buffer("train_curve", torch.zeros(300))
        self.register_buffer("val_curve", torch.zeros(300))
    ...
    def forward(self, x):
        ...
        if self.use_fc_bottleneck:
            ...
            z = F.relu(self.fc1(z))
            z = F.dropout(z, p=0.5, training=self.training)  # dropout in bottleneck
            z = F.relu(self.fc2(z))
            e4 = F.interpolate(
                z.view(B, C, self.fc_spatial, self.fc_spatial),
                size=(H, W),
                mode='bilinear',
                align_corners=False
            )
        ...
        if self.final_dropout_p > 0:
            d2 = F.dropout(d2, p=self.final_dropout_p, training=self.training)
