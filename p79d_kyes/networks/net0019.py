...
idd = 19
what = "net 18 without the inerpolation layer"
...
class main_net(nn.Module):
    def __init__(self, in_channels=1, out_channels=2, base_channels=64):
        ...
        # ----- Encoder -----
        self.enc1 = nn.Conv2d(in_channels, base_channels, kernel_size=3, stride=1, padding=1)
        self.enc2 = nn.Conv2d(base_channels, base_channels*2, kernel_size=3, stride=2, padding=1)  # downsample H/2
        self.enc3 = nn.Conv2d(base_channels*2, base_channels*4, kernel_size=3, stride=2, padding=1)  # downsample H/4
        ...
        self.dec3 = nn.Conv2d(base_channels*4, base_channels*2, kernel_size=3, padding=1)

        self.up2 = nn.ConvTranspose2d(base_channels*2, base_channels, kernel_size=2, stride=2)  # H
        self.dec2 = nn.Conv2d(base_channels*2, base_channels, kernel_size=3, padding=1)

        # Final output layer, concat with raw input
    ...
    def forward(self, x):
        ...
        # ----- Bottleneck (residual) -----
        z = F.relu(self.bottleneck(e3))
        z = z + e3  # residual connection
