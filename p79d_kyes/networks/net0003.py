...
class main_net(nn.Module):
    def __init__(self, in_channels=1, out_channels=2, base_channels=32):
        ...
        self.dec2 = nn.Conv2d(base_channels*4, base_channels, kernel_size=3, padding=1)
        self.dec1 = nn.Conv2d(base_channels*2, out_channels, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        e1 = F.relu(self.bn1(self.enc1(x)))   # [B, base, H, W]
        e2 = F.relu(self.bn2(self.enc2(self.pool(e1))))  # [B, base*2, H/2, W/2]
        e3 = F.relu(self.bn3(self.enc3(self.pool(e2))))  # [B, base*4, H/4, W/4]
        
        # Decoder
        d3 = F.relu(self.bn4(self.dec3(self.up(e3))))    # [B, base*2, H/2, W/2]
        d3 = torch.cat([d3, e2], dim=1)                 # skip connection
        
        d2 = F.relu(self.bn5(self.dec2(self.up(d3))))   # [B, base, H, W]
