...
def thisnet():
    ...
    model = main_net(hidden_dim=1024, base_channels=64, fc_spatial=16)
...
class main_net(nn.Module):
    def __init__(self, in_channels=1, out_channels=2, base_channels=32, use_fc_bottleneck=True, fc_hidden=512, hidden_dim=None, fc_spatial=4):
        super().__init__()
        ...
        if use_fc_bottleneck:
            self.fc_spatial = fc_spatial  # small spatial size for FC bottleneck
    ...
    def forward(self, x):
        ...
        # --- Encoder ---
        e1 = F.relu(self.enc1(x))       # [B, C, H, W]
        e2 = F.relu(self.enc2(self.pool(e1)))  # [B, 2C, H/2, W/2]
        e3 = F.relu(self.enc3(self.pool(e2)))  # [B, 4C, H/4, W/4]
        e4 = F.relu(self.enc4(self.pool(e3)))  # [B, 8C, H/8, W/8]
