...
idd = 28
what = "net 26, learn with FC layer off"
...
def thisnet():
    ...
    model = main_net(hidden_dim=1024, base_channels=32, fc_spatial=4, use_fc_bottleneck=False)
...
class main_net(nn.Module):
    def __init__(self, in_channels=1, out_channels=2, base_channels=32,
                 use_fc_bottleneck=True, fc_hidden=512, hidden_dim=None, fc_spatial=4):
        super().__init__()
        ...
        self.enc4 = nn.Conv2d(base_channels*4, base_channels*8, 3, padding=1)
        self.pool = nn.MaxPool2d(2)

        # --- Optional Fully Connected Bottleneck ---
        if use_fc_bottleneck:
            self.fc_spatial = fc_spatial
            self.fc1 = nn.Linear(base_channels*8 * self.fc_spatial * self.fc_spatial, fc_hidden)
            self.fc2 = nn.Linear(fc_hidden, base_channels*8 * self.fc_spatial * self.fc_spatial)

        # --- Learned Upsample + Conv ---
        self.up4_conv = nn.Conv2d(base_channels*8, base_channels*8, 3, padding=1)
        self.up3_conv = nn.Conv2d(base_channels*4, base_channels*4, 3, padding=1)
        self.up2_conv = nn.Conv2d(base_channels*2, base_channels*2, 3, padding=1)

        # --- Decoder ---
        self.dec4 = nn.Conv2d(base_channels*8 + base_channels*4, base_channels*4, 3, padding=1)
