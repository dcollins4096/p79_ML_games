...
idd = 166
what = "Equivariant, memorize.  freeze batch norm"
...
def freeze_inner_batchnorm(module):
    # keep affine learnable, just stop updating running stats
    from escnn.nn import InnerBatchNorm
    for m in module.modules():
        if isinstance(m, InnerBatchNorm):
            m.eval()
...
def thisnet():
    ...
    model = main_net(err_Bisp=0, err_Power=0, err_SSIM=0)

    model = model.to('cuda' if torch.cuda.is_available() else 'cpu')
    freeze_inner_batchnorm(model)
