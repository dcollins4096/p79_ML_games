...
class main_net(nn.Module):
    def __init__(self, in_channels=1, out_channels=2, base_channels=32, hidden_dim=512, fc_spatial=4):
        ...
        # Fully connected bottleneck (size depends on fc_spatial)
        ...
        # Decoder
        self.dec3 = nn.Conv2d(base_channels*4, base_channels*2, kernel_size=3, padding=2, dilation=2)
        self.dec2 = nn.Conv2d(base_channels*4, base_channels,kernel_size= 3, padding=2, dilation=2)
        self.dec1 = nn.Conv2d(base_channels*2, out_channels,kernel_size= 3, padding=4, dilation=4)
    ...
    def forward(self, x):
        ...
        # Flatten for FC using adaptive pooling
        B, C, H, W = e3.shape
        ...
        z = F.relu(self.fc2(z))
        z = z.view(B, C, self.fc_spatial, self.fc_spatial)                  # reshape to pooled size
