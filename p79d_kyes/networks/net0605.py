...
def trainer(
    ...
    for epoch in range(1, epochs+1):
        if epoch > 10 and save_err_Cross>0:
            model.err_Bisp = save_err_Cross
        model.train()
        if verbose:
            print("Epoch %d"%epoch)
        running = 0.0
...
class main_net(nn.Module):
    def __init__(self, in_channels=1, out_channels=3, base_channels=32,
                 ...
                 err_L1=1, err_Multi=1,err_Pear=1,err_SSIM=1,err_Grad=1,err_Power=1,err_Bisp=1,err_Cross=1,
