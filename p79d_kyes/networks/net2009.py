...
class EBFlowHead(nn.Module):
    def __init__(self, in_channels, context_dim=32, hidden_features=128, num_layers=5, edge_width=8):
        ...
        self.edge_width = edge_width  # controls taper size

        # Compress local context (encoder output)
        self.context_net = nn.Sequential(
            nn.Conv2d(in_channels + 2, context_dim, 3, padding=1),
            nn.ReLU(),
        ...
        for _ in range(num_layers):
            ...
            transform_list.append(transforms.RandomPermutation(features=3))
        self.transform = transforms.CompositeTransform(transform_list)
        self.base = distributions.StandardNormal(shape=[3])
        self.flow = flows.Flow(self.transform, self.base)
        self.context_dim = context_dim

    # --- cosine window helper ---
    def cosine_window(self, H, W, device):
        """Smooth apodization: 1 in center → 0 at edges within edge_width pixels."""
        m = self.edge_width
        ...
        win = (wy.unsqueeze(1) * wx.unsqueeze(0))
        Wmask = 1 - win  # invert so 1 center, 0 edge
        Wmask = Wmask / Wmask.mean()  # normalize to keep loss scale consistent
        return Wmask  # [H,W]
    ...
    def forward(self, features, target=None, mode="train"):
        ...
        if mode == "train":
            ...
            Wmask = self.cosine_window(H, W, features.device)
            ...
            # Power-spectrum mismatch (apodized)
...
class main_net(nn.Module):
    ...
    def criterion1(self, preds, target, epoch=10):
        ...
        flow_loss = self.flow_head(features, target=target, mode="train")
