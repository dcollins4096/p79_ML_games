...
idd = 2010
what = "2008 with a new flow head"
...
def load_data():
    ...
    all_data={'train':train['train'],'valid':valid['valid'], 'test':valid['test'][:300], 'quantities':{}}
...
class IgnoreContextNet(nn.Module):
    """Wrapper around Sequential that ignores a 'context' argument."""
...
class EBFlowHead(nn.Module):
    """
    3-channel version (T, E, B) with spline couplings and cosine edge taper.
    Drop-in replacement for EBFlowHead — same interface.
    """

    def __init__(self, in_channels, context_dim=64, hidden_features=128,
        ...
        self.context_net = nn.Sequential(
            nn.Conv2d(in_channels+2, 64, 3, padding=1, padding_mode="reflect"),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, context_dim, 3, padding=1, padding_mode="reflect"),
            nn.ReLU(inplace=True),
        )

        # --- 3D spline coupling flow ---

        def make_coupling():
            def make_net(in_features, out_features, *args, **kwargs):
                # kwargs/context ignored safely
                return IgnoreContextNet(in_features, out_features, hidden_features)
        ...
        for _ in range(num_layers):
            chain += [
                ...
                transforms.RandomPermutation(features=3),
                make_coupling(),
            ]
        ...
        # Running residual normalization (EMA)
        self.register_buffer("res_mu", torch.zeros(1, 3, 1, 1))
        self.register_buffer("res_sigma", torch.ones(1, 3, 1, 1))
        self.momentum = 0.01
    ...
    def _standardize(self, r, update_stats=True):
        if update_stats and self.training:
            mu = r.mean(dim=(0, 2, 3), keepdim=True)
            var = r.var(dim=(0, 2, 3), keepdim=True, unbiased=False)
            sigma = torch.sqrt(var + 1e-6)
            self.res_mu = (1 - self.momentum) * self.res_mu + self.momentum * mu.detach()
            self.res_sigma = (1 - self.momentum) * self.res_sigma + self.momentum * sigma.detach()
        return (r - self.res_mu) / (self.res_sigma + 1e-6)

    # -------------------------------
    # forward
    # -------------------------------
    def forward(self, features, target=None, mean_pred=None, mode="train"):
        """
        features : [B,C,H,W]  encoder/decoder context
        target   : [B,3,H,W]  ground truth maps
        mean_pred: optional [B,3,H,W] mean map (defaults to zeros)
        """
        ...
        coords = torch.stack([xx, yy]).expand(B, -1, H, W).detach()
        ...
        if mode == "train":
            ...
            if mean_pred is None:
                mean_pred = torch.zeros_like(target)

            residual = target - mean_pred
            residual_std = self._standardize(residual, update_stats=True)

            y = residual_std.permute(0, 2, 3, 1).reshape(-1, 3)
            log_prob = self.flow.log_prob(y, context=context).view(B, H, W)

            Wmask = self.cosine_window(H, W, device)
            nll = -(log_prob * Wmask).mean()
            win = Wmask[None, None]  # broadcast [1,1,H,W]

            tv = (
                (residual_std[:, :, :, 1:] - residual_std[:, :, :, :-1]).abs().mean()
                + (residual_std[:, :, 1:, :] - residual_std[:, :, :-1, :]).abs().mean()
            ) * 1e-5
            ...
            samples_std, _ = self.flow._transform.inverse(z, context=context)
            samples_std = samples_std.reshape(B, H, W, 3).permute(0, 3, 1, 2)
            samples = samples_std * (self.res_sigma + 1e-6) + self.res_mu
            if mean_pred is not None:
                samples = samples + mean_pred
            return samples
