...
idd = 612
what = "600 a ton of regularization and long training"
    ...
    for epoch in range(1, epochs+1):
        ...
        with torch.no_grad():
            ...
            for xb, yb in val_loader:
                ...
                vloss = model.criterion(preds, yb, report=True)
...
class main_net(nn.Module):
    ...
    def criterion1(self, preds, target):
        ...
        all_loss['reg_loss'] = self.reg_loss(preds)
        return all_loss

    def criterion(self, preds, target, report=False):
        losses = self.criterion1(preds,target)
        data_loss = sum(losses.values())
        if report:
            for los in losses:
                print("%10s %0.2e"%(los, losses[los]))
    ...
    def weight_l1_reg(self):
        reg = 0.0
        for name, p in self.named_parameters():
            if not p.requires_grad:
                continue
            # exclude bias + norm-style 1D params
            if p.ndim == 1 or name.endswith(".bias") or ("bn" in name.lower()) or ("norm" in name.lower()):
                continue
            reg = reg + p.abs().mean()
        return reg

    def loss_weights_reg(self):
        return self.loss_weights.pow(2).mean()  # if you want

    def reg_loss(self, preds):
        out_main = preds[0]
        lam_tv  = 1.0
        lam_lw  = 1.0
        lam_l1w = 1.0
        ...
        reg = 0.0
        reg = reg + lam_tv  * self.tv_loss(out_main)
        reg = reg + lam_lw  * self.loss_weights_reg()
        reg = reg + lam_l1w * self.weight_l1_reg()
        return reg
