...
idd = 1
what = "2d conv.  Start.  Can't memorize or really do much"
...
class main_net(nn.Module):
    def __init__(self, in_channels=1, hidden_dim=128):
        ...
        # Global pooling -> always [batch, 64]
        ...
        # Decoder (works on any H, W)
        self.deconv1 = nn.ConvTranspose2d(64, 32, kernel_size=2, stride=2)
        self.deconv2 = nn.Conv2d(32, 2, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        x = self.pool(F.relu(self.conv2(x)))   # [B, 64, H/2, W/2]
        
        # Save spatial dims
        h, w = x.shape[2:]
        
        # Global pooling -> FC -> expand back
        pooled = self.global_pool(x).view(B, -1)   # [B, 64]
        z = F.relu(self.fc1(pooled))
        z = F.relu(self.fc2(z))                   # [B, 64]
        z = z.view(B, 64, 1, 1).expand(-1, -1, h, w)
        
        # Decode
        x = F.relu(self.deconv1(z))  # [B, 32, H, W]
        x = self.deconv2(x)          # [B, 2, H, W]
