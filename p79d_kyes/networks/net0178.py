...
idd = 178
what = "168 with cross spectrum loss"
...
def cross_spectra_crit(guess,target):
    err_T = cross_spectrum_delta(guess[:,0:1,:,:], target[:,0:1,:,:])
    err_E = cross_spectrum_delta(guess[:,1:2,:,:], target[:,1:2,:,:])
    err_B = cross_spectrum_delta(guess[:,2:3,:,:], target[:,2:3,:,:])
    return err_T+err_E+err_B
...
class main_net(nn.Module):
    def __init__(self, in_channels=1, out_channels=3, base_channels=32,
                 ...
                 err_L1=1, err_Multi=1,err_Pear=1,err_SSIM=1,err_Grad=1,err_Power=1,err_Bisp=0,err_Cross=0.1,
