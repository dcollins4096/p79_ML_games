...
class main_net(nn.Module):
    ...
    def criterion1(self, preds, target):
        ...
        ssim_e  = ssim_loss(out_main[:,0:1,:,:], target[:,0:1,:,:])
        ssim_b  = ssim_loss(out_main[:,1:,:,:], target[:,1:,:,:])
        grad_e  = gradient_loss(out_main[:,0:1,:,:], target[:,0:1,:,:])
        grad_b  = gradient_loss(out_main[:,1:,:,:], target[:,1:,:,:])
        pear_e  = 1-my_pearsonr(out_main[:,0:1,:,:].flatten(), target[:,0:1,:,:].flatten())
        pear_b  = 1-my_pearsonr(out_main[:,1:,:,:].flatten(),  target[:,1:,:,:].flatten())
        lambda_pear = 2.0*(pear_e+pear_b)

        lambda_ssim = 1.0*(ssim_e+ssim_b)
        lambda_grad = 1.0*(grad_e+grad_b)


        total_loss = loss_main + 0.5 * loss_d2 + 0.25 * loss_d3 + 0.125 * loss_d4 + lambda_ssim + lambda_grad 
        stuff=[loss_main, loss_d2, loss_d3, loss_d4, lambda_ssim, lambda_grad, lambda_pear]
        out = torch.stack(stuff).to(device)
        return out

    def criterion(self, preds, target):
        losses = self.criterion1(preds,target)
        weights = torch.tensor([1,0.5,0.25,0.125,1,1,2]).to(device)
